...
class ScaffoldClient(BaseClient):
    ...
    def fit(self, parameters, config: Dict[str, Scalar]):
        ...
        if os.path.exists(f"{self.save_dir}/client_cv_{self.client_id}.pt"):
            self.client_cv = torch.load(f"{self.save_dir}/client_cv_{self.client_id}.pt")
        ...
        for c_i_j, c_j, x_j, y_i_j in zip(self.client_cv, server_cv, x, y_i):
            ...
            c_i_n.append(
                c_i_j
                - c_j
                + (1.0 / (learning_rate * epochs * len(trainloader)))
                * (x_j - y_i_j)
            )
    ...
    def train(self, net, trainloader, optim, epochs, device: str, config: dict, server_cv, client_cv):
